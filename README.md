...
* Communication/QoS options for DCC profile selection and adaptive generation are being restored incrementally. Local CBR is now available to `McService` for future adaptive MCM generation rules, but adaptive frequency reduction remains staged/WIP and disabled by default.
